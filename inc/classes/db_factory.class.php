<?php
/* $Id: db_factory.class.php 数据库工厂类 2011-12-22 13:09 聆星 $ */
...
final class db_factory {
	...
	public static function get_instance($db_config = '') {
		if($db_config == '') {
			$db_config = inc_base::load_config('database');
		}
	...
	public function connect($db_name) {
		...
		switch($this->db_config[$db_name]['type']) {
			...
			case 'mysql' :
				inc_base::load_sys_class('mysql', '', 0);
				$object = new mysql();
				break;
			case 'mysqli' :
				$object = inc_base::load_sys_class('mysqli');
				break;
			case 'access' :
				$object = inc_base::load_sys_class('db_access');
				break;
			case 'oracel' :
			
				$object = inc_base::load_sys_class('oracel', '', 0);
