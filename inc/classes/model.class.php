<?php 
/* $Id: model.class.php 数据模型基类 2011-12-22 13:09 聆星 $ */
/* @Copyright 2012 EaseMany  */

inc_base::load_sys_class('db_factory', '', 0);
class model {
...
/*	$data='a.id,a.title,a.description,a.thumb,a.inputtime';
	
	$table='`ymh_product` as a ,  `ymh_product_data` as b';
	
	$where=" a.id=b.id  and b.gllb=$linkageid  and a.siteid=$siteid ";
	
	$a=$db_product->select_sql($data, $table, $where , $pagesss, $order = '');*/
	
	final public function select_sql($data, $table, $where = '', $limit = '', $order = '', $group = '', $key='') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->select_sql($data, $table, $where, $limit, $order, $group, $key);
	}
	 ...
	 * @return unknown_type
	 */
	final public function listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 7,$urlrule = '',$array = array()) {
		$where = to_sqls($where);
		$this->number = $this->count($where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
		$this->pages = pages($this->number, $page, $pagesize, $urlrule, $array, $setpages);
		$this->pages_list = pages_list($this->number, $page, $pagesize, $urlrule, $array, $setpages);
		$array = array();
		if ($this->number > 0) {
		...
		final public function listinfo2($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array(),$url_html) {
		$where = to_sqls($where);
		$this->number = $this->count($where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
		...
		$this->newpages = newpages($this->number, $page, $pagesize, $urlrule, $array, $setpages,$url_html);
		$array = array();
		if ($this->number > 0) {
			return $this->select($where, '*', "$offset, $pagesize", $order, '', $key);
		} else {
		...
		final public function listinfo3($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 7,$urlrule = '',$array = array()) {
		$where = to_sqls($where);
		$this->number = $this->count($where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
		$this->ajax_pages = ajax_pages($this->number, $page, $pagesize, $urlrule, $array, $setpages);
		//$this->pages_list = pages_list($this->number, $page, $pagesize, $urlrule, $array, $setpages);
		$array = array();
		if ($this->number > 0) {
			return $this->select($where, '*', "$offset, $pagesize", $order, '', $key);
	...
	final public function listinfo_sql($data, $table, $where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 7,$urlrule = '',$array = array()) {
		$this->number = $this->count_sql($table, $where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
		$this->pages = pages($this->number, $page, $pagesize, $urlrule, $array, $setpages);
		$array = array();
		if ($this->number > 0) {
			return $this->select_sql($data, $table, $where, "$offset, $pagesize", $order, '', $key);
		} else {
			return array();
		}
	}
	...
	final public function get_one_sql($data, $table, $where = '', $order = '', $group = '') {
		if (is_array($where)) $where = $this->sqls($where);
		return $this->db->get_one_sql($data, $table, $where, $order, $group);
	}
	...
	final public function query($sql) {
		$sql = str_replace('ec_', $this->db_tablepre, $sql);
	...
	final public function count_sql($table, $where = '') {
		$r = $this->get_one_sql("COUNT(*) AS num",$table,$where);
		return $r['num'];
	}
