...
class form {
	...
	public static function upfiles($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$file_setting = array() ) {
		...
		if(!$alowexts) $alowexts = 'rar|zip|doc|docx';
		if(!defined('IMAGES_INIT')) {
			$str = '<script type="text/javascript" src="'.JS_PATH.'swfupload/swf2ckeditor.js"></script>';
			define('IMAGES_INIT', 1);
		}
	 ...
	 */
	public static function select_category($file = '',$catid = 0, $str = '', $default_option = '') {
		$tree = inc_base::load_sys_class('tree');
		
		
		$category_db = inc_base::load_model("$file");
	 ...
	 * @param intval $siteid 如果设置了siteid 那么则按照siteid取
	 */
	public static function select_menus($file = '',$catid = 0, $str = '', $default_option = '') {
		$tree = inc_base::load_sys_class('tree');
	...
	public static function select_template($style, $module, $id = '', $str = '', $pre = '') {
		$tpl_root = inc_base::load_config('system', 'tpl_root');
	...
	public static function checkcode($id = 'checkcode',$code_len = 4, $font_size = 20, $width = 130, $height = 50, $font = '', $font_color = '', $background = '') {
		return "<img id='$id' onclick='this.src=this.src+\"&\"+Math.random()' src='".APP_PATH."api/checkcode.php?code_len=$code_len&font_size=$font_size&width=$width&height=$height&font_color=".urlencode($font_color)."&background=".urlencode($background)."' style=\"cursor:pointer;\" title=\"换一张\">";
	}
	/**
	 * url  规则调用
	 * 
