...
class image {
    ...
    function thumb($image, $filename = '', $maxwidth = 200, $maxheight = 200, $suffix='', $autocut = 0, $ftp = 0,$c_cut = 0) {
...
/**** cut from middle ****/
    ...
    $dst_x = 0;
        $dst_y = 0;
        if ($maxwidth < $width) {
            $dst_x = round(($maxwidth - $width)/2);
        } 
        if ($maxheight < $height) {
            $dst_y = round(($maxheight - $height)/2);
        }
	   	if(function_exists('imagecopyresampled'))
			imagecopyresampled($thumbimg, $srcimg,  $dst_x,$dst_y , $psrc_x, $psrc_y, $width, $height, $srcwidth, $srcheight); 
		else
			imagecopyresized($thumbimg, $srcimg, $dst_x,$dst_y , $psrc_x, $psrc_y, $width, $height,  $srcwidth, $srcheight); 	
/**** cut from middle ****/
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'phpcms',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, CORE_PATH.'libs/data/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
