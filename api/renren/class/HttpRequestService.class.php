...
/*		$this->curl_redir_exec($this->ch, CURLOPT_USERAGENT, self::$userAgent);
		$this->curl_redir_exec($this->ch, CURLOPT_CONNECTTIMEOUT, $this->connecttimeout);
		$this->curl_redir_exec($this->ch, CURLOPT_TIMEOUT, $this->timeout);
		$this->curl_redir_exec($this->ch, CURLOPT_RETURNTRANSFER, TRUE);
		$this->curl_redir_exec($this->ch, CURLOPT_AUTOREFERER, TRUE);
        $this->curl_redir_exec($this->ch, CURLOPT_FOLLOWLOCATION, TRUE);
		$this->curl_redir_exec($this->ch, CURLOPT_HTTPHEADER, array('Expect:'));
		$this->curl_redir_exec($this->ch, CURLOPT_SSL_VERIFYPEER, $this->ssl_verifypeer);
		$this->curl_redir_exec($this->ch, CURLOPT_HEADERFUNCTION, array($this, 'getHeader'));
		$this->curl_redir_exec($this->ch, CURLOPT_HEADER, FALSE);*/
	...
	public  function curl_redir_exec($ch,$debug="") 
	{ 
		static $curl_loops = 0; 
